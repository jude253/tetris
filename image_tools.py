...
if __name__ == '__main__':
    ...
    for letter in letters:
        new_image = Image.fromarray(np.ones((50, 50, 3), dtype=np.uint8) * 255)
        image_editable = ImageDraw.Draw(new_image)
        image_editable.text((25, 25), letter, (0, 0, 0), font=font, anchor='mm')
        letter_images.append(np.array(new_image))
    new_image = Image.fromarray(np.vstack(letter_images))
    new_image.save('all_letters_small.png')
